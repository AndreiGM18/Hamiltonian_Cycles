...
int
build_hamil(list_graph_t *lg, linked_list_t *hamil, int *nr_cycles)
{
	if (hamil->size == lg->nodes) {
		int start = *(int *)get_nth_node(hamil, 0)->data;
		int end = *(int *)get_nth_node(hamil, hamil->size)->data;
		
		if (lg_has_edge(lg, end, start) && hamil->size == lg->nodes) {
			ll_add_nth_node(hamil, hamil->size, hamil->head->data);

			ll_print_int(hamil);
			
			ll_node_t *removed = ll_remove_nth_node(hamil, hamil->size - 1);
			free(removed->data);
			free(removed);
		...
		for (int i = 0; i < lg->nodes; ++i) {
			int end = *(int *)get_nth_node(hamil, hamil->size)->data;
			int pos;
			if (lg_has_edge(lg, end, i) && !find_node(hamil, i, &pos)) {
				ll_add_nth_node(hamil, hamil->size, &i);

				build_hamil(lg, hamil, nr_cycles);
...
void
hamilton(list_graph_t *lg, int src)
{
	linked_list_t *hamil = ll_create(sizeof(int));

	ll_add_nth_node(hamil, 0, &src);
